...
//        paint.setTypeface()
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        try {
            ...
            paintTextOffset = -(fontMetrics.bottom - fontMetrics.top) / 2f - fontMetrics.top;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mWeather.dailyForecast == null || mWeather.now == null) {
            return;
        }
        paint.setColor(Color.WHITE);
        ...
        try {
            ...
            canvas.drawText(mWeather.now.wind.spd + "km/h " + mWeather.now.wind.dir, fanHeight + textSize, 0, paint);
            ...
            float speed = 0f;
            try {
                speed = Float.valueOf(mWeather.now.wind.spd);
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            canvas.drawText("气压 " + mWeather.now.pres + "hpa", pressureTextRight, sunArcHeight + paintTextOffset, paint);
            // draw astor info
            final float textLeft = width / 2f - sunArcRadius;// sunArcSize;
            paint.setTextAlign(Paint.Align.CENTER);
            canvas.drawText("日出 " + mWeather.dailyForecast.get(0).astro.sr, textLeft, textSize * 10.5f + paintTextOffset, paint);
            canvas.drawText(mWeather.dailyForecast.get(0).astro.ss + " 日落", width - textLeft, textSize * 10.5f + paintTextOffset, paint);
        } catch (Exception e1) {
            e1.printStackTrace();
        }
        ...
        try {
            // draw the sun
            String[] sr = mWeather.dailyForecast.get(0).astro.sr.split(":");// 日出
            int srTime = Integer.valueOf(sr[0]) * 60 * 60 + Integer.valueOf(sr[1]) * 60 + 0;// 精确到秒
            String[] ss = mWeather.dailyForecast.get(0).astro.ss.split(":");// 日落
